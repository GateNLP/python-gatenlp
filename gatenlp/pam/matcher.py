        ...
        for fmn, fmv in self.fm.items():   # "featurematchername"/"featurematchervalue"
            ...
            if callable(fmv):
                if not fmv(feature):
                    ...
                    return False
            elif isinstance(fmv, CLASS_RE_PATTERN) or isinstance(fmv, CLASS_REGEX_PATTERN):
                fstr = str(feature)
        ...
        if self.type is not None:
            if isinstance(self.type, str):
                ...
                if not self.type(ann.type):
                    return False
            elif isinstance(self.type, CLASS_RE_PATTERN) or isinstance(self.type, CLASS_REGEX_PATTERN):
