...
The `GateCloudAnnotator` is an annotator that uses the [GATE Cloud](https://cloud.gate.ac.uk/shopfront) web service to annotate documents: for each document that gets processed, data is sent to a HTTP endpoint, processed there and information is sent back that is then used to annotate the document. 
...
<div><style>#ZVWGJBHWFZ-wrapper { color: black !important; }</style>
<div id="ZVWGJBHWFZ-wrapper">
...
.ZVWGJBHWFZ-selection {
    margin-bottom: 5px;
}

.ZVWGJBHWFZ-featuretable {
    margin-top: 10px;
}
