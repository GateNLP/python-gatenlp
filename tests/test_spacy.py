...
class TestSpacy01:
    ...
    def test_spacy03(self):
        ...
        txt = "When 2 plus 2 makes 5 then  your system is doing something wrong!. But in life 2 and 2 not always makes 4. "
        ...
        for ann in containing_set1:
            ...
            i+=1    
        gdoc = apply_spacy(nlp, doc, setname="spacy1", containing_anns=containing_set1,component_cfg="number_detector",retrieveSpans=["Numbers"])
      
        annsOut = gdoc.annset("spacy1")
        sents = annsOut.with_type("Sentence")
        assert len(sents) == 2
        nums = annsOut.with_type("Numbers")
        assert len(nums) == 6
        for ann in containing_set1:
